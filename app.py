...
RUNTIME_STRING_SESSION: Optional[str] = None
QR_LOGIN_SESSIONS: Dict[str, dict] = {}
...
def load_telegram_config() -> TelegramConfig:
    ...
    env_string_session = os.getenv("TELEGRAM_STRING_SESSION", "").strip() or None
    string_session = env_string_session or RUNTIME_STRING_SESSION
...
async def telegram_config_status():
    ...
    return {
        ...
        "session_source": "env" if os.getenv("TELEGRAM_STRING_SESSION", "").strip() else ("runtime" if RUNTIME_STRING_SESSION else "none"),
    }


async def import_telethon_modules():
    try:
...
async def build_client(cfg: TelegramConfig):
    ...
    TelegramClient, StringSession = await import_telethon_modules()
    ...
    if not await client.is_user_authorized():
        ...
        raise HTTPException(
            status_code=400,
            detail=(
                "Telegram session is not authorized. Use QR Login first, or provide TELEGRAM_STRING_SESSION, "
                "or authorize the local telegram-session file once."
            ),
        )
...
async def start_qr_login():
    cfg = load_telegram_config()
    if not cfg.api_id or not cfg.api_hash:
        raise HTTPException(status_code=400, detail="Missing TELEGRAM_API_ID or TELEGRAM_API_HASH.")
    ...
    if await client.is_user_authorized():
        ...
        await client.disconnect()
        global RUNTIME_STRING_SESSION
        RUNTIME_STRING_SESSION = session_string
        return {
            "status": "already_authorized",
            "message": "Telegram client already authorized.",
    ...
    return {
        ...
        "login_id": login_id,
        "qr_url": qr_login.url,
        "expires_at": qr_login.expires.astimezone(timezone.utc).isoformat() if qr_login.expires else None,
    }


@app.post("/api/telegram/auth/qr/check")
async def check_qr_login(payload: QRCheckRequest):
    login_state = QR_LOGIN_SESSIONS.get(payload.login_id)
    if not login_state:
        raise HTTPException(status_code=404, detail="QR login session not found or expired.")

    client = login_state["client"]
    qr_login = login_state["qr_login"]
    ...
    try:
        ...
        await client.disconnect()
        QR_LOGIN_SESSIONS.pop(payload.login_id, None)
        raise HTTPException(status_code=400, detail=f"QR login failed: {exc}") from exc
    ...
    return {
        "status": "authorized",
        "string_session": session_string,
        "message": "QR login successful. Runtime session has been activated.",
    }
