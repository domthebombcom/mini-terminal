...
# ----- API CONFIG -----
# Add any new API references here near the top of app.py.
...
async def get_crypto_candles(symbol: str = "BTCUSDT", interval: str = "1m", limit: int = 500):
    ...
    params = urllib.parse.urlencode({"symbol": symbol.strip().upper(), "interval": interval, "limit": max(10, min(int(limit), 1000))})
...
async def api_snapshot():
    out = {}
    try:
        spread = await ust_spread()
        if spread.get("series"):
            out["UST_10Y_2Y"] = spread["series"][-1]["value"]
    except Exception:
        out["UST_10Y_2Y"] = None
    try:
        data = await fred_series(ids="CPIAUCSL,UNRATE,SP500,VIXCLS,DGS10")
        series = data.get("series", {})
        out["CPI"] = (series.get("CPIAUCSL") or [{}])[-1].get("value")
        out["UNRATE"] = (series.get("UNRATE") or [{}])[-1].get("value")
        out["SP500"] = (series.get("SP500") or [{}])[-1].get("value")
        out["VIX"] = (series.get("VIXCLS") or [{}])[-1].get("value")
        out["DGS10"] = (series.get("DGS10") or [{}])[-1].get("value")
    except Exception:
        pass
    for sym in ("BTCUSDT","ETHUSDT","SOLUSDT"):
        tick = LATEST.get(sym)
        out[sym] = tick.get("last") if tick else None
    return out
